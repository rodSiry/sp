...
void addTetra(std::vector<float>* vert, std::vector<float>* nm, mat4 trans){
    vector<vec3>base;
    base.push_back(vec3(0.0f,-0.0f,-1.0f ));
    base.push_back(vec3(-1.0f,-0.0f, 1.0f));
    base.push_back(vec3(0.0f,0.5f,0.5f ));
    base.push_back(vec3(1.0f,-0.0f, 1.0f));
    base.push_back(vec3(0.0f,-0.0f,-1.0f ));
    base.push_back(vec3(0.0f,0.5f,0.5f ));
    base.push_back(vec3(-1.0f,-0.0f, 1.0f));
	...
	for(int i(0);i<base.size();i++){
		if(i%3==0)
			n=normalize(cross(base[i+1]-base[i], base[i+2]-base[i+1]));
		for(int j(0);j<3;j++){
			nm->push_back(n[j]);
			vert->push_back(base[i][j]);
		}
	}
}
